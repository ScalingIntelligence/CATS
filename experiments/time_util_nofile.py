...
class GenerationMixin:
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        print("Max length: ", stopping_criteria.max_length)
        ...
        while True:
            ...
            # if this_peer_finished and not synced_gpus:
            if count == stopping_criteria.max_length - context_length:
                end_event.record()
                torch.cuda.synchronize()
                elapsed_time_ms = start_event.elapsed_time(end_event)
                count = count - 1
                print(
                    "Context Length: {}; Generation Length: {}; Elapsed time: {} ms; Throughput: {:.2f} tokens/s".format(
                        context_length,
                        count,
                        elapsed_time_ms,
                        count / (elapsed_time_ms / 1000),
                    )
                )
                ...
                with open(save_path, "a") as f:
                    print(
                        f"{context_length},{count},{elapsed_time_ms},{count / (elapsed_time_ms / 1000)}",
                        file=f,
                    )
                break
